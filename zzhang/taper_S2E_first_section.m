...
xlambda = 1.1698e-09*(1-0.035)*(1+0.05)*0.9*1.05*1.22; %% positive taper aw0 = 2.7
Nshot = 50;
Nund = 110;
Nsec = 115;
nslice = 4000;
...
zstop = 50;
constseed = 1;
comemnts = 'S2E simulation. positive taper scan. dump dpa for next. ';
...
%             disp([num2str(timenow(4),'%d'),':',num2str(timenow(5),'%d'),':',num2str(round(timenow(6)),'%d'),': submit jobs to oak...'])
%             disp(['ssnum = ',num2str(ssnum)]);
%             [~,b] = bsub_bubble_local(foldname,128,'genesis.in');
%             disp(b)
%         end
        pause(20);
